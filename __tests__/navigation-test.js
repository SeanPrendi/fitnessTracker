...
describe('Main navigation', () => {
  ...
  it('Should render main page', () => {
    ...
    expect(wrapper.find('Text').first().props().children).toEqual('Workout Pal');
  });

  it('Should switch to app settings on button press', () => {
    const setPage = jest.fn();
    const wrapper = mount(<Main setPage={setPage} day="Back" />);
    expect(wrapper.find('Button').at(0).props().id).toEqual('appSettings');
    wrapper.find('TouchableHighlight').at(0).simulate('click');
  ...
  it('Should switch to calendar on button press', () => {
    const setPage = jest.fn();
    const wrapper = mount(<Main setPage={setPage} day="Back" />);
    expect(wrapper.find('Button').at(1).props().id).toEqual('calendar');
    wrapper.find('TouchableHighlight').at(1).simulate('click');
  ...
  it('Should switch to main on button press', () => {
    const setPage = jest.fn();
    const wrapper = mount(<Main setPage={setPage} day="Back" />);
    expect(wrapper.find('Button').at(2).props().id).toEqual('main');
    wrapper.find('TouchableHighlight').at(2).simulate('click');
  ...
  it('Should switch to user settings on button press', () => {
    const setPage = jest.fn();
    const wrapper = mount(<Main setPage={setPage} day="Back" />);
    expect(wrapper.find('Button').at(3).props().id).toEqual('userSettings');
    wrapper.find('TouchableHighlight').at(3).simulate('click');
  ...
  it('Should switch to stats on button press', () => {
    const setPage = jest.fn();
    const wrapper = mount(<Main setPage={setPage} day="Back" />);
    expect(wrapper.find('Button').at(4).props().id).toEqual('stats');
    wrapper.find('TouchableHighlight').at(4).simulate('click');
